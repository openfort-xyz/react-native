...
export function useWalletAuth(hookOptions?: WalletHookOptions) {
  ...
  const generateSiweMessage = useCallback(async (args: GenerateSiweMessageOptions): Promise<GenerateSiweMessageResult> => {
    try {
  ...
  const signInWithSiwe = useCallback(async (opts: SiweOptions): Promise<WalletHookResult> => {
    try {
      setSiweState({ status: 'submitting-signature' });
