...
- **EOA + fee sponsorship**: `createWallet` now throws a clear error when an EOA account is created while a `feeSponsorshipId` (gas policy) is configured, since fee sponsorship requires a smart account. Previously this surfaced later as a misleading "insufficient funds" error.
